        ...
        <div className="flex flex-col w-full h-full gap-1">
          <div className="flex w-full h-fit items-center justify-between">
            <div className="flex gap-2">
              ...
              <p className="text-md font-bold font-sans">Hotspot</p>
            </div>

            {/* Header */}
            <div className="flex w-fit h-fit items-enter justify-center gap-2">
              <div className={`border px-5 py-1 ${CONFIDENCE.find(e => e.id === String(selectedFire?.confidence))?.color} border-(--color-partial-stroke) rounded-md`}>
                <p className="text-sm font-sans font-medium text-background">
                 {
                  CONFIDENCE.find(e => e.id === String(selectedFire?.confidence))?.value
                 }
                </p>
              </div>
          ...
          {!isMinimized && (
            <div className="flex flex-col w-full h-full">
              ...
              <p className="text-light text-sm">{selectedFire?.acqDate}, {selectedFire?.acqTime}</p>
